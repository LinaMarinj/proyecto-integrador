...
var pJS = function (elementSelector, a) {
    // Obtener el elemento del Shadow DOM
    var container = document.querySelector("body > sidebar-menu");
    if (!container) {
        console.error("No se encontró el elemento 'sidebar-menu'");
        return;
    }

    var shadowRoot = container.shadowRoot;
    if (!shadowRoot) {
        console.error("No se encontró el Shadow Root");
        return;
    }

    // Seleccionar el canvas dentro del Shadow DOM
    var canvasElement = shadowRoot.querySelector(`#${elementSelector} > .particles-js-canvas-el`);
    if (!canvasElement) {
        console.error(`No se encontró el canvas con la clase 'particles-js-canvas-el' dentro del Shadow Root`);
        return;
    }
    this.pJS = {
        canvas: {
...
Object.deepExtend = function (e, a) {
    ...
    window.pJSDom = [],
    window.particlesJS = function (elementSelector, config) {
        if (typeof elementSelector !== 'string') {
            config = elementSelector;
            elementSelector = 'particles-js';
        }
        elementSelector = elementSelector || 'particles-js';
        ...
        // Seleccionar el contenedor del canvas dentro del Shadow DOM
        var canvasContainer = shadowRoot.querySelector(`#${elementSelector}`);
        if (!canvasContainer) {
            console.error(`No se encontró el elemento con ID '${elementSelector}' dentro del Shadow Root`);
            return;
        }
    
        // Eliminar antiguos canvas
        var oldCanvases = canvasContainer.querySelectorAll('.particles-js-canvas-el');
        oldCanvases.forEach(canvas => canvas.remove());
    
        // Crear un nuevo canvas
        var newCanvas = document.createElement('canvas');
        newCanvas.className = 'particles-js-canvas-el';
        newCanvas.style.width = '100%';
        newCanvas.style.height = '100%';
        canvasContainer.appendChild(newCanvas);
    
        // Configurar el plugin de partículas
        if (typeof pJSDom === 'undefined') {
            pJSDom = [];
        }
        pJSDom.push(new pJS(elementSelector, config));
    }    
    ,
